...
require("lazy").setup({
  ...
  -- following line to the REPL, like we would do with other vim operators.
  {
    ...
    opts = {
      ...
      config = {
        ...
        -- cells.
        repl_open_cmd = "vertical botright 90 split",
        --repl_open_cmd = function()
        --	return require("iron.view").center("30%", 20)
        --end,
        ...
        repl_definition = {
          ...
          python = {
            ...
            command = function()
              local bin = require("venv-selector").venv() .. "/bin/ipython"
              return { bin, "--no-autoindent", "--nosep" }
            end,
          },
        },
      },
  ...
  -- f-strings
  -- - auto-convert strings to f-strings when typing `{}` in string.
  -- - will also convert them back if the `{}` are removed.
  -- {
  ...
  -- LLMs ----------
  -- Handle plugins associated with code completions etc.
  {
    "yetone/avante.nvim",
    event = "VeryLazy",
    lazy = false,
...
-- Make sure to have some basic installations into Mason so
-- it stops complaining. Even though we don't want these in all
-- cases, we want to make sure that neovim doesn't crap out on
...
-- DISABLING DAP -- I haven't found a use for this yet, really.
-- Is just noise at this point, I just do the manual debug thing.
--
-- Make sure to have dap UI configured
